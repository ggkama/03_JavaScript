/* (중요!!)
 HTML문서에서 얻어온 값의 자료형은 무조건 String(문자열)
 */
 ...
 /* Number(input1.value)가 계속 반복되는데
   ...
   -> 안된다 !!
    ...
    why? 전역 변수는 페이지 로딩 시점에 해석되서 값이 결정된다.
   ...
   const test2 = 0; // Number(input1.value);
...
function plusFn(){
    ...
    // 단, '숫자형태 문자열'이 아닐 경우 NaN(Not a Number) 결과 변환
...
function allOperationFn(){
    ...
    const v1 = Number(input3.value); // 첫번째 값
    const v2 = Number(input4.value); // 두번째 값
...
// ex) +,-,*,/,%, ++, --,
//     변수에 값 대임, console 출력, innerText ... 모두 다
...
function checkFn(){
    ...
    num = 100;
    console.log("num++ : ", num++); // 100 출력 -> num1 증가(101)

    console.log("num++ : ", num++); // 101 출력 -> num1 증가(102)

    console.log("num++ : ", num++); // 102 출력 -> num1 증가(103)
